...
def get_encoder(cfg):
    if cfg.type == 'frequency':
        ...
        from lib.networks.encoding.hashencoder.hashgrid import HashEncoder
        encoder = HashEncoder(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'cuda_hashgrid_latent':
        from lib.networks.encoding.hashencoder.hashgrid import HashLatent
        encoder = HashLatent(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'cuda_hashgrid_4d':
        from lib.networks.encoding.hashencoder.hashgrid import HashEncoder4d
        encoder = HashEncoder4d(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'cuda_hashgrid_coef':
        from lib.networks.encoding.hashencoder.hashgrid import HashEncoderCoef
        encoder = HashEncoderCoef(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'cuda_triplane':
        from lib.networks.encoding.hashencoder.hashgrid import TriPlane
        encoder = TriPlane(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'cuda_motion2d':
        from lib.networks.encoding.hashencoder.hashgrid import Motion2d
        encoder = Motion2d(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'cuda_dnerf_ngp_tensorf':
        from lib.networks.encoding.hashencoder.hashgrid import DNeRFNGP
        encoder = DNeRFNGP(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'dnerf_ngp_tensorf':
        from lib.networks.encoding.hashgrid import DNeRFNGP
        encoder = DNeRFNGP(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'dnerf_ngp_mlp':
        from lib.networks.encoding.hashgrid import DNeRFNGP_MLP
        encoder = DNeRFNGP_MLP(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'dnerf_mlp_tensorf':
        from lib.networks.encoding.hashgrid import DNeRFTensoRF
        encoder = DNeRFTensoRF(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'triplane':
        from lib.networks.encoding.triplane import TriPlane
        encoder = TriPlane(**cfg)
        return encoder, encoder.out_dim
    elif cfg.type == 'dnerf':
        from lib.networks.encoding.dnerf import DNeRF
        encoder = DNeRF(**cfg)
        return encoder, encoder.out_dim
